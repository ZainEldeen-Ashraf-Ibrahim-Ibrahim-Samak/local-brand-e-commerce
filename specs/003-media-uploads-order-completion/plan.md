...
control and wires it into the admin product, category, and homepage-offer forms — backed by the existing
secure Cloudinary signing endpoint — with type/size limits, ordering, a publish gate (a product needs
≥1 image to publish), and immediate cleanup of removed/orphaned assets; (2) makes the homepage slider
render active offer slides with their images (and a placeholder fallback); and (3) resolves incomplete
orders via a scheduled background sweep that expires unpaid pending orders past a window and restores
their reserved stock idempotently, with late-payment reconciliation that never oversells.

Technical approach: extend the existing Next.js App Router project in place. The data models already
carry image fields (`Product.images[]`, `Category.image`, `Offer.image`) and the order lifecycle already
has a terminal `failed` status with stock restoration on `failOrder` — so most work is UI, validation,
a Cloudinary delete helper, and a scheduled expiry sweep. No new third-party dependency is introduced.
...
Image fields already exist on Product/Category/Offer. Cloudinary stores the hosted assets.
...
**Scale/Scope**: 3 user stories (all P1); FR-201–FR-217; 1 new reusable component (MediaUploader); small
edits to 3 admin forms + their service/route validation; 1 Cloudinary delete helper; order expiry sweep
+ 2 additive Order fields; homepage/card placeholder fallbacks.
...
| IV | Role-Based Access & Guest-Friendly Commerce | Image management restricted to admin (and buyer for own products); order expiry is system-driven and never blocks guests | PASS — role guards unchanged; sweep is backend |
...
│   │   └── CategoryManager.tsx          # MODIFIED — single category image via MediaUploader
...
├── services/
│   ├── admin/catalog.admin.service.ts   # MODIFIED — publish-requires-image gate; delete removed/replaced assets
...
(Principle I); a shared `ImageWithFallback` guarantees no broken images. Order expiry is a domain-service
function (`expireStaleOrders`) invoked by a thin, secret-gated `app/api/cron/expire-orders` route so the
host scheduler can drive it without coupling business logic to the trigger.
