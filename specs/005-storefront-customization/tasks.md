...
- [X] T001 Create new source folders per plan: `src/lib/favorites/`, `src/lib/compare/`, `src/components/storefront/`, `src/components/admin/content/`, `src/components/admin/catalog/`, `tests/integration/`, `tests/unit/`
- [X] T002 [P] Add next-intl message keys placeholders for new surfaces (hero, filters, favorites, compare, badges, privacy, terms, currency) in `src/i18n` message catalogs (en + ar)
- [X] T003 [P] Verify Vitest + MongoDB Memory Server config covers new `tests/integration` and `tests/unit` paths in the existing test config
...
- [X] T004 Extend `WebsiteSettings` model in `src/models/WebsiteSettings.ts` — add `privacyPage.body`, `termsPage.body`, `hero{ background, heading, subtext, cta{label,href}, showHeading, showSubtext, showCta }`, `currency{ base, active, options:[{code,label,symbol,rate}] }`, `homeSections:[{key,isVisible,sortOrder}]` (all additive, defaults preserved)
- [X] T005 [P] Extend `Offer` model in `src/models/Offer.ts` — add `placement: "hero" | "offer"` (default `"offer"`, indexed)
- [X] T006 Extend cached reads in `src/services/settings.service.ts` so hero/currency/privacyPage/termsPage/homeSections are returned from the cached settings singleton
- [X] T007 Extend `src/services/admin/settings.admin.service.ts` to accept the new settings fields and invalidate both `settings` and `home` caches on write
- [X] T008 [P] Create shared Zod schemas for content/hero/currency/slider/category request bodies in `src/lib/validation/content.ts` (reused by admin routes in US1/US2/US7/US8)
...
- [X] T010 [US1] Implement `PUT /api/admin/content` in `src/app/api/admin/content/route.ts` — admin guard + Zod (T008) + `updateWebsiteSettings` (T007)
- [X] T011 [P] [US1] Build `src/components/admin/content/ContentManager.tsx` — header/nav/footer/homeSections + About/Contact/Privacy/Terms editor using `components/ui` primitives + design tokens
- [X] T012 [US1] Create admin content page `src/app/[locale]/(admin)/admin/content/page.tsx` rendering ContentManager (shared shell extended by US2/US8)
- [X] T013 [P] [US1] Create storefront `src/app/[locale]/(storefront)/privacy/page.tsx` rendering `privacyPage.body` with "not available" empty state
- [X] T014 [P] [US1] Create storefront `src/app/[locale]/(storefront)/terms/page.tsx` rendering `termsPage.body` with empty state
- [X] T015 [US1] Ensure storefront header/footer/nav components read nav links + footer columns from settings (modify existing storefront layout/footer/header components as needed)
...
- [X] T018 [US2] Implement `GET/POST /api/admin/sliders` in `src/app/api/admin/sliders/route.ts` (placement-aware) + `PUT/DELETE /api/admin/sliders/[id]` in `src/app/api/admin/sliders/[id]/route.ts` — admin guard + Zod (T008) + invalidate `home`
- [X] T019 [US2] Extend `src/services/home.service.ts` to return hero config + `heroSlides`/`offerSlides` split by `placement`
- [X] T020 [US2] Extend `PUT /api/admin/content/hero` (in `src/app/api/admin/content/route.ts` or `.../hero/route.ts`) to save hero config; reuse existing signed upload `POST /api/admin/media/sign` for background
- [X] T021 [P] [US2] Build `src/components/admin/content/HeroEditor.tsx` — background upload (signed) + component visibility/content toggles
- [X] T022 [P] [US2] Build `src/components/admin/content/SliderManager.tsx` — hero/offer slides CRUD + reorder
- [X] T023 [US2] Wire HeroEditor + SliderManager into the admin content page `src/app/[locale]/(admin)/admin/content/page.tsx`
- [X] T024 [P] [US2] Build storefront `src/components/storefront/HeroSection.tsx` — full-bleed background with toggleable overlay components (AR/RTL + dark/light)
- [X] T025 [US2] Render HeroSection + hero/offer sliders on the storefront home page consuming the extended home payload
...
- [X] T027 [US3] Extend `src/services/catalog.service.ts` `listProducts` — resolve descendant category `_id` set for a selected parent slug; add `subCategories` to `facets`
- [X] T028 [US3] Extend `GET /api/storefront/products` in `src/app/api/storefront/products/route.ts` to surface `facets.subCategories` and the active `currency` block
- [X] T029 [P] [US3] Build `src/components/storefront/FilterPanel.tsx` — category, sub-category, price range, size, color; active-filter display + reset
- [X] T030 [US3] Wire FilterPanel + no-results/reset state into the storefront catalog page and bind filters to URL query params
- [X] T031 [P] [US3] Add AR/EN strings for filters + no-results state in `src/i18n`
...
- [X] T032 [P] [US4] Unit test `tests/unit/useFavorites.test.ts` — add/remove/persist + `lb_fav_change` event + count

### Implementation for User Story 4

- [X] T033 [P] [US4] Create `src/lib/favorites/useFavorites.ts` — localStorage hook (`lb_fav_v1`) mirroring `useCart`, emits `lb_fav_change`
- [X] T034 [P] [US4] Build `src/components/storefront/FavoriteButton.tsx` — toggle + active state (shared component + tokens)
- [X] T035 [US4] Create `src/app/[locale]/(storefront)/favorites/page.tsx` — list with links; resolve against published catalog and omit unavailable items (FR-018)
- [X] T036 [US4] Wire FavoriteButton into product card + product detail components
...
- [X] T037 [P] [US5] Unit test `tests/unit/useCompare.test.ts` — max-3 guard, add/remove, `lb_cmp_change` event

### Implementation for User Story 5

- [X] T038 [P] [US5] Create `src/lib/compare/useCompare.ts` — localStorage hook (`lb_cmp_v1`), max 3, full-list guard, emits `lb_cmp_change`
- [X] T039 [P] [US5] Build `src/components/storefront/CompareButton.tsx` — toggle + full-list message
- [X] T040 [US5] Create `src/app/[locale]/(storefront)/compare/page.tsx` — side-by-side comparable attributes; omit unavailable items (FR-018)
- [X] T041 [US5] Wire CompareButton into product card + product detail components
...
- [X] T042 [P] [US6] Build shared `src/components/ui/CountBadge.tsx` — numeric badge using design tokens (reused by cart/favorites/compare)
- [X] T043 [US6] Wire CountBadge to `useCart`, `useFavorites` (T033), `useCompare` (T038) in the storefront header/nav; ensure event + `storage` listeners keep counts accurate across tabs/reloads
...
- [X] T045 [US7] Create category admin service in `src/services/admin/categories.admin.service.ts` — CRUD with parent validation (existing parent, no cycles) + invalidate `categories` cache
- [X] T046 [US7] Implement `GET/POST /api/admin/categories` in `src/app/api/admin/categories/route.ts` + `PUT/DELETE /api/admin/categories/[id]` in `src/app/api/admin/categories/[id]/route.ts` — admin guard + Zod (T008); reject delete when sub-categories/products attached (409)
- [X] T047 [P] [US7] Build `src/components/admin/catalog/SubCategoryManager.tsx` — nested category CRUD + product assignment
- [X] T048 [US7] Create admin categories page `src/app/[locale]/(admin)/admin/categories/page.tsx`
...
- [X] T050 [P] [US8] Create `src/services/currency.service.ts` — resolve active currency from cached settings + convert base minor units by stored rate
- [X] T051 [US8] Extend `src/lib/format.ts` `formatMoney` to apply active currency + rate (default to base when unconfigured)
- [X] T052 [US8] Implement `PUT /api/admin/currency` in `src/app/api/admin/currency/route.ts` — admin guard + Zod (T008, active∈options & rate>0) + invalidate `settings`+`home`
- [X] T053 [P] [US8] Build `src/components/admin/content/CurrencyForm.tsx` — active currency + per-currency rate editor; wire into admin content page
- [X] T054 [US8] Apply currency-aware display across catalog/product/cart/checkout price surfaces using `formatMoney` + active currency
